...
public class furnitureGet {
	...
	@Nullable
	public static ArmorStand getFromLocation(Location location,@Nullable BlockFace face) {
		ArmorStand ea = null;
		if(face != null) {
			Location blockc = location.getBlock().getRelative(face).getLocation();
			...
			for(Entity instance : furnitureGet.getFromBoundingBox(bb,blockc.getWorld())) {
				if(instance instanceof ArmorStand) {
					if(CustomFurniture.byAlreadySpawned((ArmorStand) instance) != null){
						ea = (ArmorStand) instance;
					}
				}
			}
		}else {
